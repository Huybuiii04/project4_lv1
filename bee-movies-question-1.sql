...
(select count(*) from names ) as name,
...
from  genre
group by genre)
select genre , total as highest_number_of_movies
from count_genre
...
max(median_rating) as max_mdedian_rating 
from ratings
...
m.country like '%USA%' and r.total_votes > 1000
group by g.genre;
...
-- Type your code below:
select count(*) as number_of_movie_median_rating_of_8
from ratings r
join movie m on r.movie_id = m.id 
...
sum(case when known_for_movies is null then 1 else 0 end) as knowm_for_movies_nulls 
from names ;
...
-- (Hint: The top thrave the most number of movies with an average rating > 8.)
/* Output format:
